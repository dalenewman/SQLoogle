...
namespace Sqloogle.Libs.DBDiff.Schema.SqlServer2005.Generates.SQLCommands
{
    internal static class TableSQLCommand
    {
        ...
        {
            return "/* SQLoogle */ SELECT Count(*) FROM sysobjects SO WHERE type = 'U'";
        }

        private static string GetTableCount2005()
        {
            return "/* SQLoogle */ SELECT Count(*) from sys.tables";
        }
        ...
        private static string GetTableDetailAzure()
        {
            ...
            sql.AppendLine(" '' AS FileGroup, '' AS FileGroupText, '' AS FileGroupStream,ISNULL(DC.object_id,0) AS DefaultId, DC.name AS DefaultName, DC.definition AS DefaultDefinition, C.rule_object_id, C.default_object_id, T.create_date, T.modify_date ");
        ...
        private static string GetTableDetail2005()
        {
            ...
            sql += "ISNULL(DC.object_id,0) AS DefaultId, DC.name AS DefaultName, DC.definition AS DefaultDefinition, C.rule_object_id, C.default_object_id, T.create_date, T.modify_date ";
