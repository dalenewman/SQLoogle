...
namespace Sqloogle.Libs.DBDiff.Schema.SqlServer2005.Generates
{
    public class GenerateStoreProcedures
    {
        ...
        private static int createDateIndex = -1;
        private static int modifyDateIndex = -1;
        ...
        private static void InitIndex(SqlDataReader reader)
        {
            if (NameIndex == -1)
            {
                ...
                createDateIndex = reader.GetOrdinal("create_date");
                modifyDateIndex = reader.GetOrdinal("modify_date");
            }
        }
        ...
        {
            string sql = "";
            sql += "SELECT ISNULL(CONVERT(varchar,AM.execute_as_principal_id),'CALLER') as ExecuteAs, P.type, AF.name AS assembly_name, AM.assembly_class, AM.assembly_id, AM.assembly_method, P.object_id, S.name as owner, P.name as name, P.create_date, P.modify_date ";
